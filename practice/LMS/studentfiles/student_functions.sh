...
function check_available(){
	...
	then
		...
		echo "Would you like to checkout now?"
		declare -x sub_request
		read sub_request
		...
		then
			echo "Sure. Thanks for checking out on shell library!!!"
		else 
			echo "I didn't get it!!! Have a nice day!!!"
			exit
		fi
	elif [ $available = "no" ]
	then
...
function search(){
	echo "Type the name of the book you would like to search for:"
	read search_book
	cat libsheet.txt | grep -i $search_book 
...
function check_in(){
	...
	then
		...
		return_date=`cat  libsheet.txt | grep -i $book_name | awk '{print $7}'` 
		sed 's/$return_date/$today_date/'
		prev_name=`cat libsheet.txt | grep -i $book_name | awk '{print $9}'`
		sed 's/$prev_name/NA/'
		prev_id=`cat libsheet.txt | grep -i $bookname | awk '{print $8}'`
		sed 's/$prev_id/NA'
	fi
}
...
function check_out(){
	...
	then
		...
		prev_name=`cat libsheet.txt | grep -i book_checkout | awk '{print $9}'`
		sed 's/$prev_name/$student_name/'
		prev_date=`cat libsheet.txt | grep -i book_checkout | awk '{print $6}'`
		sed 's/$prev_date/$issuedate/'
		prev_id=`cat libsheet.txt | grep -i book_checkout | awk '{print $8}'`
